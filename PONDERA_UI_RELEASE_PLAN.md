...
- Existing uncommitted work predating the UI phases:
  - `Pondera/Pondera.xcodeproj/project.pbxproj`
  - `Pondera/Pondera.xcodeproj/xcshareddata/xcschemes/Pondera.xcscheme`
  - `Pondera/Pondera/App/SubscriptionManager.swift`
  - `Pondera/Pondera/Pondera.entitlements`
- Those changes configure StoreKit/platform support and Debug premium access. Preserve them.
- `Pondera/Pondera/AI/Secrets.swift` is local and gitignored. Never print, commit, or expose its values.
...
> Continue Attune from `PONDERA_REMAINING_WORK.md`,
> `PONDERA_UI_RELEASE_PLAN.md`, and `backend/openai-proxy/V2_API.md`. First
